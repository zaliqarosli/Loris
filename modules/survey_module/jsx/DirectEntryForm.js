...
class Page extends React.Component {
  ...
  render() {
    const DirectEntryFormElements = this.props.elements.map((element, key) => {
      return (
...
class SelectElement extends React.Component {
  ...
  render() {
    ...
    for (let key in this.props.element.Options.Values) {
      if (this.props.element.Options.Values.hasOwnProperty(key)) {
        let checked;
        if (key === '') {
          continue;
...
class HeaderElement extends React.Component {
  ...
  render() {
    let element;
    if (this.props.element.Type == 'header') {
        element = (
          <h1>
            <Markdown content={this.props.element.Description} />
          </h1>
