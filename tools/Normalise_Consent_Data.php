...
foreach ($consentList as $consentName=>$consentLabel) {
    ...
    // Check data
    $dataQuery = "SELECT ID, CandID, " . $consentName . ", " . $consentDate . ", " . $consentWithdrawal . " FROM participant_status";
    ...
    foreach ($psData as $entry) {
        ...
        if(empty($entry[$consentName]) && (!empty($entry[$consentDate]) || !empty($entry[$consentWithdrawal]))) {
            array_push($errors, "Consent status not found for
                       [ID]     => " . $entry['ID'] . "
                       [CandID] => " . $entry['CandID'] . " where there are dates.");
        }
        // Check if consent status is yes but consent date is empty
        if($entry[$consentName] === "yes" && empty($entry[$consentDate])) {
        ...
        if($entry[$consentName] === "no" && empty($entry[$consentWithdrawal])) {
            array_push($errors, "The date of withdrawal is miss for 
                       [ID]     => " . $entry['ID'] . "
                       [CandID] => " . $entry['CandID'] . "
